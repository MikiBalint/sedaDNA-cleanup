...
#### Hypothesis: read numbers are influenced by the DNA concentration, extraction kit and the age of the sediment.

Fit the full model.
```{r}
read.conc.kit.order.age = 
  lmer(reads ~ conc + kit + 
         extract_order + age +
         (1|depth.nominal),
     data = ExpLab)
...
read.conc.kit.age = 
  lmer(reads ~ conc + kit + age +
         (1|depth.nominal),
     data = ExpLab)

anova(read.conc.kit.order.age,
      read.conc.kit.age)
```

Visualize the variable effects
```{r}
plot(allEffects(read.conc.kit.order.age,
                multiline=TRUE, confidence.level = 0.95))
```
...
### Hill's 1 (richness)
No DNA concentration included since no reason that should influence the richness. The kit was already shown by others that it influences community observations.
```{r}
hill1.read.kit.order.nucl = 
  lmer(HillLab$hill1 ~ reads + kit + 
         extract_order + nuclear +
         (1|depth.nominal),
     data = ExpLab)
...
anova(hill1.read.kit.order.nucl,
      hill1.read.kit.nucl)
...
```{r include = FALSE}
plot(allEffects(hill1.read.kit.order.nucl,
                multiline=TRUE, confidence.level = 0.95))
```
...
Fit the full model.
```{r}
hill2.read.kit.order.nucl = 
  lmer(HillLab$hill2 ~ reads + kit + 
         extract_order + nuclear +
         (1|depth.nominal),
     data = ExpLab)
...
anova(hill2.read.kit.order.nucl,
      hill2.read.kit.nucl)
...
```{r include = FALSE}
plot(allEffects(hill2.read.kit.order.nucl,
                multiline=TRUE, confidence.level = 0.95))
```
...
Fit the full model.
```{r}
hill3.read.kit.order.nucl = 
  lmer(HillLab$hill3 ~ reads + kit + 
         extract_order + nuclear +
         (1|depth.nominal),
     data = ExpLab)
...
anova(hill3.read.kit.order.nucl,
      hill3.read.kit.nucl)
...
Visualize the variable effects
```{r include = FALSE}
plot(allEffects(hill3.read.kit.order.nucl,
                multiline=TRUE, confidence.level = 0.95))
```
...
I need a general, community-level statistic. `mvabund` cannot deal with random effects: I ommit the horizon effect completely.
...
# full model fit
OTU.horizon.read.kit.order.nucl =
  manyglm(OTU.mva ~ depth.nominal + reads + kit + 
            extract_order + nuclear,
...
ANOVA test of effects
```{r eval=FALSE}
# OTU.anova = anova(OTU.horizon.read.kit.order.nucl,
#                   nBoot = 10, test = "LR")
```

Variations explained and statistical significances
```{r eval=FALSE}
# OTU.anova$table
```
