...
Linear model first
```{r}
lm1 = lm(conc ~ weight + kit + person + 
           extract_order + DDE.4.4 + nuclear,
         data = ExpLab)
```

```{}
plot(allEffects(lm1, multiline=TRUE, confidence.level=0.95))
```

Fit the full model
```{r}
conc.weight.kit.person.order.age = 
  lmer(conc ~ weight + kit + person + 
       extract_order + age + I(age^2) + 
         (1|depth.nominal),
     data = ExpLab)
```

Visualize the variable effects
```{r include = FALSE}
plot(allEffects(conc.weight.kit.person.order.age,
                multiline=TRUE, confidence.level = 0.95))


```

the standard errors are lower for the extract_order than for the person
```{r}
summary(conc.weight.kit.person.order.age)
```

Fit without person. The more complex model is not better.
```{r}
conc.weight.kit.order.age = 
  lmer(conc ~ weight + kit + 
       extract_order + age + I(age^2) +
         (1|depth.nominal),
     data = ExpLab)

anova(conc.weight.kit.person.order.age,
      conc.weight.kit.order.age)
```

Model results
```{r}
summary(conc.weight.kit.order.age)
```

Visualize the variable effects
```{r include = FALSE}
plot(allEffects(conc.weight.kit.order.age,
                multiline=TRUE, confidence.level = 0.95))
```
![](conc_effect.png)

The extraction_order explains as much variation as the age.
```{r}
anova(conc.weight.kit.order.age)
```
...
#### Hypothesis: read numbers are influenced by the DNA concentration, extraction kit, extracting person and the  age of the sediment.

Fit the full model.
```{r}
read.conc.kit.person.order.age = 
  lmer(reads ~ conc + kit + 
         extract_order + person + age +
         (1|depth.nominal),
     data = ExpLab)
```
...
Visualize the variable effects
```{r include = FALSE}
plot(allEffects(read.conc.kit.person.order.age,
                multiline=TRUE, confidence.level = 0.95))
```

Model summary: the standard errors of the extraction order are higher than those for the person
```{r}
summary(read.conc.kit.person.order.age)
```

Simpler model without the order. The complex model is not better.
```{r}
read.conc.kit.person.age = 
  lmer(reads ~ conc + kit + 
         person + age +
         (1|depth.nominal),
     data = ExpLab)

anova(read.conc.kit.person.order.age,
      read.conc.kit.person.age)
```

Visualize the variable effects
```{r include = FALSE}
plot(allEffects(read.conc.kit.person.age,
                multiline=TRUE, confidence.level = 0.95))
```
![](read_effect.png)

The model with the person is stat. significantly better than a simpler one.
```{r}
read.conc.kit.age = 
  lmer(reads ~ conc + kit + 
         age + (1|depth.nominal),
...
anova(read.conc.kit.person.age,
      read.conc.kit.age)
```

### Richness
#### Hypothesis: read numbers, DNA concentration, extraction kit, extracting person, extraction order, DDE and nuclear power plant operation explain the richness.

The full model:
```{r}
rich.read.conc.kit.person.order.DDE.nuc = 
  lmer(HillLab$hill1 ~ reads + conc + 
...
Visualize the variable effects
```{r include = FALSE}
plot(allEffects(rich.read.conc.kit.person.order.DDE.nuc,
                multiline=TRUE, confidence.level = 0.95))
```

Model summary. The kit has a largest relative standard error.
```{r}
summary(rich.read.conc.kit.person.order.DDE.nuc)
```

Simpler model with no kit. Complex model is not better.
```{r}
rich.read.conc.person.order.DDE.nuc = 
  lmer(HillLab$hill1 ~ reads + conc + 
...
anova(rich.read.conc.kit.person.order.DDE.nuc,
      rich.read.conc.person.order.DDE.nuc)
```

Visualize the variable effects
```{r include = FALSE}
plot(allEffects(rich.read.conc.person.order.DDE.nuc,
                multiline=TRUE, confidence.level = 0.95))
```
![](rich_effects.png)

The laboratory factors explain comparable variation in richness as the biologically interesting predictors, DDE and nuclear power plant.
```{r}
anova(rich.read.conc.person.order.DDE.nuc)
```
...
#### Hypothesis: Hill's 2 is explained by read numbers, DNA concentration, extraction kit, extracting person, extraction order, DDT, nuclear.

Fit the full model.
```{r}
hill2.conc.kit.person.order.DDT.nucl = 
  lmer(HillLab$hill2 ~ reads + conc + kit + 
         extract_order + person + DDE.4.4 + nuclear +
         (1|depth.nominal),
     data = ExpLab)
```
...
Visualize the variable effects
```{r include = FALSE}
plot(allEffects(hill2.conc.kit.person.order.DDT.nucl,
                multiline=TRUE, confidence.level = 0.95))
```


Kit doesn't seem important.
```{r}
hill2.conc.person.order.DDT.nucl = 
  lmer(HillLab$hill2 ~ reads + conc + 
         extract_order + person + DDE.4.4 + nuclear +
         (1|depth.nominal),
     data = ExpLab)
```

Complex model not better, kit not justified.
```{r}
anova(hill2.conc.kit.person.order.DDT.nucl,
      hill2.conc.person.order.DDT.nucl)
```

Effect plots
```{r include = FALSE}
plot(allEffects(hill2.conc.person.order.DDT.nucl,
                multiline=TRUE, confidence.level = 0.95))
```
![](hill2_effect.png)
...
Model summary
```{r}
summary(hill2.conc.person.order.DDT.nucl)
```

Hill2 variation partitions
```{r}
anova(hill2.conc.person.order.DDT.nucl)
```
...
NMDS visualization of the community composition
```{r}
...
# ordispider(MDS.all, ExpLab$nuclear, col="red")
points(MDS.all, pch=19, col = as.numeric(ExpLab$depth)*10)
